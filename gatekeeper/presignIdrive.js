...
export async function createPresignedIdriveUrl({ env = {}, operation, key, contentType = "application/octet-stream", contentLength = 0 } = {}) {
  ...
  const objectKey = normalizeObjectKey(key, { operation });
  ...
  const bucket = resolveBucketForKey(objectKey, env);
...
// Maus-Replay Stufe A (2026-07-15): Lauf-Artefakte liegen im Capsule-Bucket
// (z. B. smejj-app), nicht im Standard-Bucket des Control Servers. Rein
// additiv und fail-closed: Nur der bereits download-only freigegebene Prefix
// capsules/maus-engine/ nutzt IDRIVE_E2_CAPSULES_BUCKET; ohne diese Env-Variable
// bleibt das Verhalten byte-identisch zum bisherigen Standard-Bucket.
function resolveBucketForKey(objectKey, env) {
  const capsulesBucket = String(env.IDRIVE_E2_CAPSULES_BUCKET || "").trim();
  if (capsulesBucket && String(objectKey).startsWith("capsules/maus-engine/")) {
    return capsulesBucket;
  }
  return String(env.IDRIVE_E2_BUCKET);
}
