export const GATEKEEPER_POLICY = Object.freeze({
  ...
  paidObjectStorageAllowed: false,
...
// Nur-Lese-Prefixe (Maus-Replay Stufe A, 2026-07-15, schriftlich freigegeben):
// ausschliesslich fuer operation "download" zusaetzlich erlaubt. Upload auf
// diese Prefixe bleibt blockiert (fail-closed); bestehende Prefixe unveraendert.
const DOWNLOAD_ONLY_KEY_PREFIXES = /^capsules\/maus-engine\//;

export function normalizeObjectKey(key, { operation } = {}) {
  ...
  if (operation === "download" && DOWNLOAD_ONLY_KEY_PREFIXES.test(value)) {
    return value;
  }
  return null;
}
