export function runFreeAppExecutor({ task = "", jobEnvelope = null, now = new Date().toISOString() } = {}) {
  ...
  return {
    ...
    memory: {
      learn: false,
      status: allPassed ? "blocked_until_memory_rights_privacy_quality_gate" : "blocked_until_verified_success",
    ...
    training: {
      eligible: false,
      state: "denied",
      reasons: ["legacy-free-executor-source", "separate-training-pipeline-required"]
    },
    worker: {
...
function buildReportObjects({ rootPrefix, artifactPrefix, projectSlug, artifactFiles, tests, browserSmoke, allPassed, now }) {
  ...
  return [
    ...
    },
    {
      ...
      contentType: "application/json; charset=utf-8",
      body: `${JSON.stringify({ version: 1, learn: false, state: "legacy-source-memory-blocked", trainingEligible: false, source: `${artifactPrefix}final-report.md`, evidence: [`${artifactPrefix}test-results.json`, `${artifactPrefix}browser-results.json`], createdAt: now }, null, 2)}\n`
    },
    {
      path: "training-eligibility.json",
      key: `${rootPrefix}training-eligibility.json`,
      contentType: "application/json; charset=utf-8",
      body: `${JSON.stringify({ schemaVersion: 1, trainingEligible: false, state: "denied", reasons: ["legacy-free-executor-source", "separate-training-pipeline-required"], createdAt: now }, null, 2)}\n`
    }
  ];
}
