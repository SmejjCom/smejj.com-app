...
test("shouldSearchWeb trifft bei aktuellen/faktischen Fragen zu", () => {
  assert.equal(shouldSearchWeb("Wetter heute in Berlin"), true);
  assert.equal(shouldSearchWeb("Was ist die Hauptstadt von Australien?"), true);
  assert.equal(shouldSearchWeb("Aktuelle Nachrichten"), true);
  assert.equal(shouldSearchWeb("Deutschland, Einwohnerzahl"), true);
