...
class TransformersWeg:
    ...
    def antworte(self, fall, max_tokens):
        tok = self.tok
        try:
            kodiert = tok.apply_chat_template(_nachrichten(fall), add_generation_prompt=True,
                                              return_tensors="pt", enable_thinking=False)
        except TypeError:
            kodiert = tok.apply_chat_template(_nachrichten(fall), add_generation_prompt=True, return_tensors="pt")
        # transformers 5.x liefert ein BatchEncoding (dict-artig, hat .to aber kein .shape) —
        # gemessen 03.09.: 46 leere Antworten in 8 ms, weil das dict als Eingabe durchging.
        if hasattr(kodiert, "keys"):
            input_ids = kodiert["input_ids"]
            attention = kodiert.get("attention_mask")
        else:
            input_ids = kodiert
            attention = None
        input_ids = input_ids.to(self.modell.device)
        kw = {"input_ids": input_ids, "max_new_tokens": max_tokens, "min_new_tokens": 1, "do_sample": False,
              "pad_token_id": tok.pad_token_id if tok.pad_token_id is not None else tok.eos_token_id}
        if attention is not None:
            kw["attention_mask"] = attention.to(self.modell.device)
        with self.torch.no_grad():
            out = self.modell.generate(**kw)
        prompt_len = int(input_ids.shape[-1])
        seq = out[0] if hasattr(out, "shape") else out.sequences[0]
        # Volle Sequenz (Prompt + neu) ist der Normalfall; liefert ein Modell nur die neuen Tokens, nicht abschneiden.
