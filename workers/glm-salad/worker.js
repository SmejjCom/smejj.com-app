...
export async function runWorkerTask(config, input, io = defaultIo) {
  ...
  const memoryEntry = ok ? buildSolvedErrorMemoryEntry(config, { task, patchVerification, browserResults, selfFixAttempts, benchmarkResults }) : null;
  return {
    ok,
    testResults: {
      ok,
    ...
    finalReport: [
      ...
      `Memory update is proposal-only until verification passes.`
    ].join("\n"),
    memoryUpdate: {
      ok,
      learn: ok,
      source: "verified-worker-result",
      proposals: ok ? [{ type: "worker-success", model: config.modelVaultId, task, memoryKey: memoryEntry.key }] : []
    },
    memoryEntry
  };
}
...
function buildSolvedErrorMemoryEntry(config, { task, patchVerification, browserResults, selfFixAttempts, benchmarkResults }) {
  const jobId = config.jobId || taskCapsuleJobId(config.taskCapsulePrefix);
  const projectId = config.projectId || "project_unknown";
  const key = `projects/${projectId}/solved-errors/${jobId}.json`;
  return {
    key,
    value: {
      version: 1,
      jobId,
      projectId,
      sourceCapsule: config.taskCapsulePrefix,
      task,
      modelId: config.modelId,
      modelVaultId: config.modelVaultId,
      evidence: [
        `${config.taskCapsulePrefix}test-results.json`,
        `${config.taskCapsulePrefix}browser-results.json`,
        `${config.taskCapsulePrefix}verifier-report.md`,
        `${config.taskCapsulePrefix}benchmark-results.json`
      ],
      patch: {
        status: patchVerification.status,
        files: patchVerification.files.map((file) => ({
          path: file.path,
      ...
      browser: {
        ...
        required: browserResults.required === true
      },
      selfFix: {
        attempts: selfFixAttempts.attempts.length,
        stoppedBecause: selfFixAttempts.stoppedBecause
      },
      benchmarks: benchmarkResults.metrics,
      learnOnlyBecause: "verified_worker_success",
...
async function buildVerificationChecks({ config, input, task, runtimeReady, selfFixAttempts, patchVerification }) {
  ...
  const checks = [
    ...
    { name: "memory-learns-only-after-verification", ok: true }
  ];
