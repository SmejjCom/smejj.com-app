// muuny AI — Dashboard (Single Responsibility: Zustand aus e2 einsammeln und anzeigen; entscheidet nichts).
...
export async function baueStatus({ konfig, e2, salad }) {
  ...
  return {
    ...
    registry: { stable: registry.stable || null, candidate: registry.candidate || null, versionen: (registry.versions || []).map((v) => ({ version: v.version, herkunft: v.herkunft || null, status: v.status, gesamt: v.benchmarks?.gesamt ?? null, kritisch: v.benchmarks?.kritisch ?? null,
    ...
    deploy: { alias: deploy.alias || "muuny-stable", stable: deploy.stable, canary: deploy.canary, canaryAnteil: deploy.canaryAnteil || 0,
      canarySeit: deploy.canarySeit, wartetAuf: deploy.canaryWartetAuf || null,
      letzteBefoerderung: deploy.letzteBefoerderung || null, letzterRollback: deploy.letzterRollback || null },
...
export function dashboardHtml(s) {
  ...
  const zeilen = s.registry.versionen.map((v) => `<tr><td><b>${esc(v.version)}</b><br><small>${esc(v.herkunft && v.herkunft !== v.version ? "aus " + v.herkunft : "")}</small></td><td class="${esc(v.status)}">${esc(v.status)}</td><td>${pct(v.gesamt)}</td><td>${v.kritisch ?? "–"}</td>${kats.map((k) => `<td>${pct(v.kategorien?.[k]?.score)}</td>`).join("")}<td>${v.leistung?.tokensProSekunde ?? "–"} tok/s · ${v.leistung?.latenzMsMittel ?? "–"} ms · Fehler ${pct(v.leistung?.fehlerrate)}</td><td>${v.kostenUsd ?? "–"} USD</td><td>${esc(v.hardware || "–")}</td></tr>`).join("");
  ...
  return `<!doctype html><html lang="de"><head><meta charset="utf-8"><meta http-equiv="refresh" content="60"><title>muuny AI</title>
...
<p><small>Stand ${esc(s.zeit)} · Seite laedt jede Minute neu · JSON: /api/muuny/status</small></p></body></html>`;
}
