...
// (/admin/radar-berichte.json, ausgeliefert aus public/radar/berichte.json)
// von derselben Herkunft; die Konsole braucht dafuer
...
(function () {
  ...
  async function laden(ctx) {
    if (!daten) {
      try {
        const antwort = await fetch("/admin/radar-berichte.json", { headers: { Accept: "application/json" } });
