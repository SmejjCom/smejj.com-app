...
// ueber diesen Laeufer erreichbar — bestehende Aufrufer und Tests aendern sich nicht.
export { laufNachweisKette, laufMedienQualitaet, laufVoiceRegion };
...
export async function laufeAlle({ melde = meldeUndMerke, dateienLader = sammleQuelldateien, seitenLader = sammleSeiten, mitNetz = true, zeitlimitMs = ZEITLIMIT_JE_LAUF_MS } = {}) {
  ...
  const laeufe = [
    ...
    ["nachweis-kette", () => laufNachweisKette({ mitNetz })],
    ["process-reward", S.alsBaustein(() => S.laufProcessReward())],
    ...
    ["autonomous-git-bot", S.alsBaustein(() => S.laufGitBot())],
