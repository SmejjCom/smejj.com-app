...
export function detectLiveInternetIntent(task = "") {
  ...
  if (WEATHER_PATTERNS.test(text)) {
    return { live: true, kind: "weather", location: extractWeatherLocation(text), dayOffset: extractWeatherDayOffset(text) };
  }
...
export async function answerLiveIntent(intent, task, { fetchImpl = fetch, now = new Date() } = {}) {
  if (intent.kind === "weather") return weatherAnswer(intent.location, { fetchImpl, now, dayOffset: intent.dayOffset || 0 });
...
async function weatherAnswer(location, { fetchImpl, now, dayOffset = 0 }) {
  ...
  const source = `Quelle: Open-Meteo (${weatherUrl.origin}). Stand: ${current.time || iso(now)}.`;
  if (dayOffset > 0) {
    // Vorhersage fuer morgen/uebermorgen aus den daily-Feldern (Index = Tagesversatz).
    if (daily.temperature_2m_max?.[dayOffset] === undefined) {
      return { ok: false, answer: failClosed(`Keine Vorhersage fuer den angefragten Tag in ${placeLabel} verfuegbar.`, now) };
    }
    const dayName = dayOffset === 1 ? "morgen" : "uebermorgen";
    const date = daily.time?.[dayOffset] ? ` (${daily.time[dayOffset]})` : "";
    const answer = [
      `Wettervorhersage fuer ${dayName}${date} in ${placeLabel}: ${weatherLabel(daily.weather_code?.[dayOffset])}.`,
      `Temperatur: ${fmt(daily.temperature_2m_min?.[dayOffset])} bis ${fmt(daily.temperature_2m_max?.[dayOffset])} °C.`,
      `Regenwahrscheinlichkeit: max. ${fmt(daily.precipitation_probability_max?.[dayOffset])} %, Niederschlag gesamt: ${fmt(daily.precipitation_sum?.[dayOffset])} mm, Wind bis ${fmt(daily.wind_speed_10m_max?.[dayOffset])} km/h.`,
      `Aktuell in ${hit.name}: ${fmt(current.temperature_2m)} °C, ${weatherLabel(current.weather_code)}.`,
      "",
      source
    ].filter(Boolean).join("\n");
  ...
  const answer = [
    `Aktuelles Wetter fuer ${placeLabel}: ${weatherLabel(current.weather_code)}.`,
...
function extractWeatherLocation(text) {
  ...
  return cleanupText(match?.[1] || "Berlin")
    // Umlaut-Variante zuerst ohne \b, denn \b greift vor "ü" (Nicht-ASCII) nicht.
    .replace(/übermorgen|uebermorgen/gi, "")
    .replace(/\b(heute|jetzt|aktuell|morgen|gleich|abends|mittags|nachts)\b/gi, "")
    .replace(/^\s*(?:in|fuer|für)\b\s*/i, "")
    .trim() || "Berlin";
}

// Tagesversatz aus der Frage: 0 = heute (Standard), 1 = morgen, 2 = uebermorgen.
// Hinweis: \b greift vor "ü" nicht (Nicht-ASCII), daher Substring-Pruefung fuer beide Schreibweisen.
function extractWeatherDayOffset(text) {
  ...
  if (/(?<!guten\s)(?<!am\s)\bmorgen\b/.test(value)) return 1;
  return 0;
}
