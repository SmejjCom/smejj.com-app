...
- **Runde 26 (v975/v976):** Begrüßung im Code-Bereich übersetzt (Betreiber-Freigabe 24.09.), `t("Was steht als Nächstes an, {name}?")` in 14 Sprachen, Modell-Menü-Sperre gestempelt. v975 enthielt einen TDZ-Fehler (lokales `const t` in `zeichne()` überdeckte `t()`, gefunden von der Versionswache), ~15 min live; v976 benennt die Variable um (`tiefeAnzeige`), Test (10). Live gemessen: englisch „What's next, Messung?“, keine Fehler. Anker `schutz-100-2026-09-24-app-v976`.
- **Runde 27 (v984) + Brücke v177 (25.09.):** Offene Punkte aus dem Auftrag geschlossen. (a) Brücke v177: Eine `bildErneut`-Nachfrage WÄHREND des Malens wartet auf dieses Malen, statt ein zweites zu starten (Ashburn-Maler 85–165 s; Obergrenze 6 min, Lebenszeichen alle 10 s). Anker `schutz-100-2026-09-25-bruecke-v177`. (b) Punkt 7 „kein Erfolg ohne Bild“: Bei Abriss fällt „Hier ist dein Bild:“ weg (`entferneAbgerisseneMedien`), bei Ladefehler wird die Ankündigung ausgeblendet und kehrt zurück, sobald das Bild lädt (`erfolgszeile`). (c) Neustart-Rettung wartet bis 240 s. Test (11). Anker `schutz-100-2026-09-25-app-v984`.
- **Bild-Stresstest (Punkt 11), 25.09., iPhone-Simulator 17 Pro Max, Build 5, live v984:** 10 Aufträge nacheinander (Apfel, Bergdorf, Segelboot, Welpe, Skyline, Kaffee, Leuchtturm, Ramen, Ballons, Bibliothek), je 85–110 s. **10/10 erzeugt und angezeigt, 10/10 nach hartem App-Neustart weiter sichtbar**, je genau eine Aktionsleiste, kein „?“, kein Hinweis. Bild 1 kam als Vektorgrafik (SVG-Reserve), weil der Maler in dem Moment umgestellt wurde. Nebenbefund: Bei offener Tastatur scrollt der Verlauf nicht bis zum neuen Bild mit (Bild steht darunter, Wischen genügt).
