...
# Klickliste (ergänzt 2026-09-06)

Abzuarbeiten von oben nach unten. Nach den ersten **drei** ist die Kette bei
5 Gliedern und die Warnung der Umgebungs-Wache verstummt.

| # | Anbieter | Schlüssel holen bei | Variable für Zeabur |
|---|---|---|---|
| 1 | **Google Gemini** | `aistudio.google.com/apikey` | `SMEJJ_LLM_GEMINI_API_KEY` |
| 2 | **Cerebras** | `cloud.cerebras.ai` | `SMEJJ_LLM_CEREBRAS_API_KEY` |
| 3 | **Mistral** | `console.mistral.ai/api-keys` | `SMEJJ_LLM_MISTRAL_API_KEY` |
| 4 | OpenRouter | `openrouter.ai/keys` | `SMEJJ_LLM_OPENROUTER_API_KEY` |
| 5 | SambaNova | `cloud.sambanova.ai/apis` | `SMEJJ_LLM_SAMBANOVA_API_KEY` |
| 6 | NVIDIA | `build.nvidia.com` | `SMEJJ_LLM_NVIDIA_API_KEY` |
| 7 | Together | `api.together.xyz/settings/api-keys` | `SMEJJ_LLM_TOGETHER_API_KEY` |

Adressen geprüft am 06.09.2026. Ändert ein Anbieter seine Seite, findet man den
Schlüssel dort immer unter „API keys" in den Kontoeinstellungen.

## Die eine Regel beim Anmelden

**Keine Karte, keine Zahlungsdaten hinterlegen.** Ohne hinterlegtes
Zahlungsmittel kann kein Anbieter eine Rechnung stellen — er hört auf zu
antworten, wenn das Gratiskontingent alle ist. Genau das soll er auch: der
Router merkt den Fehlschlag und nimmt das nächste Glied.

## Eintragen im Zeabur-Portal

Dienst **smejj-control** → Variables → Variable + Wert → speichern → **Redeploy**.

Ein Neustart genügt NICHT. Der Router liest die Umgebung einmal beim Start und
baut daraus die Kette; ohne neuen Bau bleibt die alte Umgebung stehen.

## Prüfen, ob es gewirkt hat

```
curl -s https://smejj-control.zeabur.app/api/health
```

Die Kettenlänge steht in jeder Meldung der Umgebungs-Wache (Nr. 71).
