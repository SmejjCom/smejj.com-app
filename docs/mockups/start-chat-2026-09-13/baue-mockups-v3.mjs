// smejj.com — Mockups "Startseite + Chat neu", VORSCHLAG 3 (13 Bildschirme, alle heutigen Knoepfe) (2026-09-13): alle heutigen Icon-Plaetze bleiben.
...
function schreibfeldRechner({ text = "", modell = "smejj 1" } = {}) {
  return `<div class="schreibfeld" style="flex-direction: row; align-items: center; gap: 8px; padding: 8px 10px;">
    ...
    ${text ? `<div style="flex: 1; font-size: 18px; padding: 0 4px;">${text}<span style="display: inline-block; width: 2px; height: 20px; background: #32f6ea; vertical-align: -4px; margin-left: 1px;"></span></div>` : `<div class="platzhalter" style="padding: 0 4px;">Frag mich alles</div>`}
...
function verlauf({ breit = true, bild = true } = {}) {
  ...
  return `<div class="col" style="gap: 20px;">
    ...
    <div class="col" style="gap: 8px;">${codeblock(breit)}${ak}</div>
    ${frage("Nenne drei Farben.")}
    <div class="col" style="gap: 8px;"><div class="antwort">Blau, Rot, Grün.</div>${ak}</div>
...
function kameraModus() {
  return `<div style="position: relative; display: flex; width: 1440px; height: 900px; overflow: hidden;">
    ...
    <div class="col" style="flex: 1; min-width: 0; padding: 70px 0 22px;">
      <div class="col" style="width: 880px; margin: 0 auto; gap: 14px; flex: 1;">
...
<div style="position: relative; display: flex; width: 1440px; height: 900px; overflow: hidden;">
  ${spur({ modus: "start", chatAktiv: 0 })}
  <div class="col" style="flex: 1; min-width: 0;">
    <div style="flex: 1; overflow: hidden; padding: 70px 0 0;"><div style="width: 880px; margin: 0 auto;">${verlauf()}</div></div>
    <div class="col" style="gap: 10px; padding: 12px 0 22px;">
      <div style="width: 880px; margin: 0 auto;">${schreibfeldRechner({ text: "Und jetzt als Video" })}</div>
...
const ChatMenues = doc(`
<div style="position: relative; display: flex; width: 1440px; height: 900px; overflow: hidden;">
  ${spur({ modus: "start", chatAktiv: 0 })}
  <div class="col" style="flex: 1; min-width: 0;">
    <div style="flex: 1; overflow: hidden; padding: 70px 0 0;"><div style="width: 880px; margin: 0 auto;">${verlauf({ bild: false })}</div></div>
    <div class="col" style="gap: 10px; padding: 12px 0 22px;">
...
const Sprachmodus = doc(sprachmodus());
const KameraModus = doc(kameraModus());
...
<div style="position: relative; display: flex; width: 1440px; height: 900px; overflow: hidden;">
  ...
  <div style="display: grid; grid-template-columns: minmax(0, 1fr) 470px; flex: 1; min-width: 0; padding-top: 60px;">
    <div class="col" style="gap: 16px; padding: 10px 28px 24px; border-right: 1px solid rgba(255,255,255,0.075); position: relative;">
      ...
      <div style="display: flex; gap: 8px; max-width: 880px;">${CODE_VORLAGEN.map(([i, t]) => `<div class="werkzeug klein" style="min-height: 40px; padding: 0 12px; font-size: 14.5px;">${ic(i, 18)}<span>${t}</span></div>`).join("")}</div>
      <div style="position: absolute; left: 28px; bottom: 200px;">${codePlusMenue()}</div>
    </div>
    ${browserFenster({ reiterAktiv: 0 })}
  </div>
...
const SprachmodusHandy = doc(sprachmodus({ breite: 390, hoehe: 844, handy: true }));
...
// ----------------------------------------------------------- Ausgabe
const dateien = {
  "Main.dc.html": StartDesktop, "Sendeknopf.dc.html": Sendeknopf, "ChatDesktop.dc.html": ChatDesktop, "ChatMenues.dc.html": ChatMenues,
  "Sprachmodus.dc.html": Sprachmodus, "KameraModus.dc.html": KameraModus, "CodeDesktop.dc.html": CodeDesktop, "SpurZu.dc.html": SpurZu,
  "StartHandy.dc.html": StartHandy, "ChatHandy.dc.html": ChatHandy, "SprachmodusHandy.dc.html": SprachmodusHandy, "MenueHandy.dc.html": MenueHandy, "ChatQuer.dc.html": ChatQuer
};
...
const canvas = {
  artboards: [
    ...
    { file: "Sendeknopf.dc.html", title: "2 · Ein Knopf, zwei Zustände: Welle ↔ Senden", x: 1560, y: 0, w: 960, h: 300 },
    ...
    { file: "SpurZu.dc.html", title: "8 · Chat mit geschlossener Spur (Logo + Schriftzug)", x: 1560, y: 3060, w: 1440, h: 900 },
