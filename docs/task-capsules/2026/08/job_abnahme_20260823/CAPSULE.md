...
## Nachtrag 16:45Z — Deploy-Wächter gebaut (Betreiber-Wahl „Deploy-Wächter bauen“)
`npm run check:deploy-abgleich` (scripts/check-deploy-abgleich.mjs): vergleicht vor dem Frontend-Kopieren jede Quelle in public/ mit origin/main des Frontend-Repos, ?v=-Marken und CACHE_NAME ausgeklammert. Nur-live-Zeilen > 3 = STOPP („live ist neuer als lokal — Live-Fassung zur Basis nehmen“). Ausnahme mit Grund: chat-bridge.js (erzeugtes Bündel). Selbsttest nach Wächter-TÜV: 4 eingesetzte nur-live-Zeilen → Stopp erkannt; reiner Markenunterschied → still. Aufruf mit Dateiliste für den eigenen Deploy: `node scripts/check-deploy-abgleich.mjs index.html sw.js chat-stopp.js`.
**Erster Volllauf fand echte Drift (nicht von mir, gehört den Parallelsitzungen):** `maus-chrome.js` 5 Zeilen nur live (Maus-Brücke; live ≠ beide Zweige), `chat-bridge-rag.js` 4 Zeilen nur live, Hinweise: `chat-bridge-strom.js` (live = Bau-Zweig — Bridge deployt aus dem anderen Zweig), `shared/http-json.js`, `en/privacy.html`. Diese Quellen sind älter als live — wer sie als Nächstes deployt, löscht fremde Arbeit. Bitte von den jeweiligen Sitzungen zurückschreiben lassen.
