...
Re-verified read-only on 2026-07-10:
...
- `npm run idrive:verify-kimi` passed for all `86` files: object presence, byte size and every available upstream SHA-256 entry match.
- Reported upstream size is `595,204,984,507` bytes (`554.3 GiB`).
- Metadata, config, tokenizer, license, third-party notices, inventory and checksum manifest are present.
- Runtime activation remains feature-flagged; IDrive e2 is storage, not compute.
...
- Local secret file: `~/.config/smejj.com/env.local` with directory mode `700`
  and file mode `600`, outside the synchronized project
- S3 access check: passed with `npm run idrive:check`

The full model-weight set is present in IDrive e2. No model weights are stored in GitHub or moved to local project storage.

A dedicated access key named `smejj-kimi-k27-storage` has been created for the transfer workflow. It is scoped to bucket `smejj-model-files` and configured for read/write access without bucket deletion or governance-bypass permissions. Real key values must stay only in injected environment variables or the secure local file outside the synchronized project.

The exact public model source for Kimi K2.7 Code is confirmed as `moonshotai/Kimi-K2.7-Code` on Hugging Face.

Current transfer status: complete and verified. The following transfer notes are retained as historical audit records.
...
Preparation status: complete. Transfer commands remain gated so the verified vault cannot be overwritten or repeated accidentally from this undersized local machine.
...
- Cloudflare.com is not used.
- GitHub must not store model weights, media archives, user data, or large generated artifacts.
...
- Do not add paid GitHub services, Cloudflare services, or other paid add-on services to this workflow.
- GitHub Free is not the inference or large-data scaling layer; it remains limited to source code and GitHub Pages static delivery.
