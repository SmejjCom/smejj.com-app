...
### C2 — Der admin-lock war schon vorher rot (nicht von mir)

```
admin-lock VERLETZT (2)
  - control-server/src/admin/stepUp.js: VERAENDERT
  - control-server/src/routes/adminUiRoutes.js: VERAENDERT
```

Beide Dateien sind **committet** und im Arbeitsbaum unverändert — die Änderung kam
mit Commit `922f9da` und wurde nie neu eingefroren. Nach der Regel *„Sperre prüfen,
nicht stempeln"* habe ich **nicht** neu eingefroren. Das ist eine Entscheidung des
Betreibers: erst ansehen, was sich geändert hat, dann freigeben.
