...
function applySelectedModel(model, { persist = true, quiet = false } = {}) {
  const selectedModel = MODEL_MODES[model] ? model : "smejj 1.0";
...
async function submitTask(task, { target = "#startLog" } = {}) {
  ...
  try {
    ...
    output.textContent = error.message || "Aufgabe konnte nicht abgeschlossen werden.";
    hideTaskIndicator();
  }
}
...
function bindAi() {
  $("#evaluateAiMode").addEventListener("click", () => {
    ...
    writeOutput("#aiOutput", JSON.stringify({
      ...
      kimiK27: "vault/byok/partner/self-host-later-only"
    }, null, 2));
  });
