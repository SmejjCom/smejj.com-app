...
import { renderChatMarkdown } from "/assets/chat-markdown.js?v=4";
// Papierkorb & Projekte/Bereiche: chat-store-bereiche.js (Diaet 25.08.); Re-Export = EINE Instanz.
import { aktualisiereBereichsAnweisung, verbraucheBereichVormerkung, BEREICH_ANWEISUNG_KEY, BEREICH_NEU_KEY } from "./chat-store-bereiche.js?v=31";
export { PAPIERKORB_TAGE, restoreChat, endgueltigLoeschen, listGeloeschteChats, listEigeneChatsMitGeloeschten, listProjekte, getProjekt, erstelleProjekt, benenneProjektUm, setzeProjektAnweisung, neuesGespraechImBereich, loescheProjekt, setzeChatProjekt, importProjekt } from "./chat-store-bereiche.js?v=31";
...
async function medienAuslagern() {
  try {
    ...
    const { lagereMedienAus, lagereMedienAusText, lagereMedienAusTextknoten } =
      await import("./chat-medien.js?v=15");
...
async function medienHolen(log) {
  try {
    const { rehydriereMedien } = await import("./chat-medien.js?v=15");
...
async function parkerBereit(messages) {
  ...
  parkerLaedt = import("./chat-medien.js?v=15")
  ...
  try {
    ...
    enforceChatOwner()
      ...
      .then((restoreErlaubt) => {
        ...
        import("/assets/chat-sync.js?v=34").catch(() => {});
      });
  } catch {
