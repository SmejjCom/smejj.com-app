...
async function medienAuslagern() {
  try {
    ...
    const { lagereMedienAus, lagereMedienAusText, lagereMedienAusTextknoten } =
      await import("./chat-medien.js?v=4");
...
async function medienHolen(log) {
  try {
    const { rehydriereMedien } = await import("./chat-medien.js?v=4");
...
import("./chat-medien.js?v=4").then((m) => { if (typeof m.parkeMedienAdressen === "function") parkeMedien = m.parkeMedienAdressen; }).catch(() => {});
