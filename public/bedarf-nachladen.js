...
//     wenige Millisekunden nach dem Start wieder her.
import("./entwurf-erhalt.js?v=1").catch(() => {});
...
if (location.pathname.includes("papierkorb")) {
  import("./papierkorb.js?v=39");
} else {
  ladeBeiKlick(['[data-view="papierkorb"]', '[data-jump="papierkorb"]'], () => import("./papierkorb.js?v=39"));
}
...
//    Liste SOFORT aufgeht, nicht erst beim naechsten Zeichen.
{
  ...
  if (feld) {
    const wecker = () => {
      ...
      import("./erwaehnung.js?v=29").then(() => feld.dispatchEvent(new Event("input", { bubbles: true })))
...
//    Weitere mit; der hiesige loest sich auf, wenn beide Stufen geladen sind.
{
  ...
  const ladeCodeWerkzeuge = () => Promise.all([
    import("./chat-code-copy.js?v=zcode3-20260816-2-2"),
...
{
  const laden = () => import("./chat-history-view.js?v=g20260924012607-2-3");
...
if (location.pathname.includes("arbeitsbereiche") || location.pathname.includes("projects")) {
  import("./arbeitsbereiche.js?v=46");
} else {
  ladeBeiKlick(['[data-view="arbeitsbereiche"]', '[data-jump="arbeitsbereiche"]'], () => import("./arbeitsbereiche.js?v=46"));
}
