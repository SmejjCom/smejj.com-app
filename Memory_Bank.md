...
Nachtrag v910: Nicht-HTML (Bild, Text, PDF, ZIP) blieb im Panel leer — `/api/browser/page` liefert jetzt eine Dateiansicht (Bild vom Original, Text maskiert, sonst Karte mit EINEM Knopf in eine neue Registerkarte; der Server liest nie Bilder/Archive). Live nachgewiesen.
...
Nachtrag 20.09. (SW v924, Geraetetest v922): mechanisches t()-Umschreiben setzte title=t("…") OHNE ${} in Template-Attribute — Adressleiste zeigte ALLEN `t("Suchen`; behoben + Waechter tests/i18n-attribut-waechter.test.mjs. iPhone-App: .browser-panel top:0/padding:0 lag unter der Statusleiste -> padding-top env(safe-area-inset-top).
Nachtrag 20.09. (SW v917): Seitentitel + Ueberschriften der Ansichten in 14 Sprachen — view-title.js uebersetzt beim Ansichtswechsel, deutsche Quelle bleibt gemerkt, index.html unberuehrt (Start-Lock, SEO). Lokale Probe braucht echte Pages-Kopie (public/assets ist nur ein Teil-Spiegel).
Nachtrag 20.09. (SW v915/v916, Geraetetest Android + iPhone): Nachrichten-Menue und 'Inhalt melden' blieben in jeder Sprache deutsch -> 29 Texte in 14 Sprachen, Uebersetzung beim Einhaengen am BODY; v914 hatte 4 Module ohne neue Marke; iPhone: klebendes button:hover aus styles.css faerbte die beruehrte Dialog-Zeile weiss -> eigene :hover-Regeln. Messfalle: App laedt nach Lieferung selbst neu, erst ansehen, dann tippen.
Nachtrag 20.09. (SW v913): v912 der Parallelarbeit verlangte live fehlende /assets/inhalt-melden.js (404) — Precache alles-oder-nichts, v912 installierte sich NIRGENDS, alle hingen auf v910. Runde 9 lieferte Melde-Funktion + Erfassungs-Fix (Herkunft doppelt) + Sicherheits-Lock-Stempel zusammen; live: Cache 243, Meldung kommt an, 0 CSP-Verstoesse. Lehre: nach jeder sw.js-Lieferung JEDE Datei der Offline-Liste live abrufen.
Nachtrag 20.09. (SW v905): Worker-Aktion `viewport` — Fenstergroesse der laufenden Live-Sitzung aendern statt neuer Chrome; live 0,73 s statt 6-10 s, Klassenwechsel Handy/Desktop wird abgelehnt (Rueckfall alter Weg). Der Fern-Browser-Dienst baut bei JEDEM Push auf den Bauzweig selbst neu. Kaskade mit BASIS = Stand nach der Zusammenfuehrung liefert nur die eigene Runde aus (Apple-Login blieb draussen).
