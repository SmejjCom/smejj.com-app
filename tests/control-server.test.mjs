...
import { hasLocalIdriveConfig, handleJobStatus, handleJobEvents, persistFreeExecutorToIdrive } from "../control-server/src/routes/jobRoutes.js";
import { handleStoragePresign } from "../control-server/src/routes/storagePresignRoutes.js";
import { encodeS3Key, boundedNumber, getS3Dates, parseS3Keys } from "../control-server/src/storage/s3Signer.js";
...
test("handleJobStatus returns 404 for unknown job and 200 with capsule plan for known job", () => {
  clearJobs();
  const notFound = fakeRes();
  handleJobStatus(new URL("http://127.0.0.1/api/jobs/missing"), notFound);
  ...
  const found = fakeRes();
  handleJobStatus(new URL(`http://127.0.0.1/api/jobs/${envelope.job.id}`), found);
...
test("handleJobEvents streams initial status and live updates via SSE", () => {
  clearJobs();
  saveJob({ id: "job_test_4", status: "pending" });
  const res = fakeRes();
  const req = fakeReq();
  handleJobEvents(new URL("http://127.0.0.1/api/jobs/job_test_4/events"), req, res);
...
test("handleJobEvents returns 404 for unknown job", () => {
  clearJobs();
  const res = fakeRes();
  handleJobEvents(new URL("http://127.0.0.1/api/jobs/nope/events"), fakeReq(), res);
