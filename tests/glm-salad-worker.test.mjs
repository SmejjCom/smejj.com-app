...
test("GLM Salad worker processes Task Capsule through IDrive IO abstraction", async () => {
  ...
  assert.equal(writes.has("projects/project_smejj/solved-errors/job_test_001.json"), false);
  assert.equal(writes.get("jobs/2026/06/26/aa/job_test_001/memory-update.json").learn, false);
  assert.equal(writes.get("jobs/2026/06/26/aa/job_test_001/memory-update.json").state, "legacy-worker-memory-denied");
});
