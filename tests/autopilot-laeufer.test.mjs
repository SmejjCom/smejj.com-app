...
test("Der Laeufer betreibt alle Selbsttest-Autopiloten und meldet jeden einzeln", async () => {
  ...
  assert.equal(ergebnisse.length, 75, "75 Autopiloten laufen ohne Netz im Control-Server (seit 2026-09-24 die Messlatte Nr. 88, seit 2026-09-22 die DNS-Wache Nr. 87, seit 2026-09-21 smejj ai radar Nr. 86; seit 2026-08-26 Trainings-Reife Nr. 65, seit 2026-08-30 die fünf Deckungs-Wächter Nr. 66-70, seit 2026-09-03 der Modell-Evolutions-Takt Nr. 72, seit 2026-09-04 die Schutz-Echtheit Nr. 82, seit 2026-09-05 der smejj-Versions-Takt Nr. 83 und die Webhook-Wache Nr. 84, seit 2026-09-08 die Code-Sicherung Nr. 85)");
  assert.equal(gemeldet.length, 76, "75 Laeufe + der Taktgeber, der sich selbst bezeugt");
  assert.equal(new Set(gemeldet.map((g) => g.id)).size, 76, "keine Kennung doppelt");
...
test("Ein abstuerzendes Modul reisst den Lauf nicht mit", async () => {
  ...
  assert.equal(ergebnisse.length, 75, "alle anderen laufen trotzdem");
...
test("Mit Netz kommt der E2E-Waechter dazu — und meldet ehrlich, wenn er nicht pruefen kann", async () => {
  ...
  assert.equal(ergebnisse.length, 78, "E2E-Waechter, Voice-Region und Sync-Waechter sind die drei zusaetzlichen Netz-Laeufe (plus Nr. 65, die fuenf Deckungs-Waechter Nr. 66-70, Nr. 72, die Schutz-Echtheit Nr. 82, die Webhook-Wache Nr. 84 und die Code-Sicherung Nr. 85 ohne Netz)");
