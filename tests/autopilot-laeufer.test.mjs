...
test("Der Laeufer betreibt alle Selbsttest-Autopiloten und meldet jeden einzeln", async () => {
  ...
  assert.equal(ergebnisse.length, 70, "70 Autopiloten laufen ohne Netz im Control-Server (seit 2026-08-26 Trainings-Reife Nr. 65, seit 2026-08-30 die fünf Deckungs-Wächter Nr. 66-70, seit 2026-09-03 der Modell-Evolutions-Takt Nr. 72, seit 2026-09-04 die Schutz-Echtheit Nr. 82, seit 2026-09-05 der smejj-Versions-Takt Nr. 83)");
  assert.equal(gemeldet.length, 71, "70 Laeufe + der Taktgeber, der sich selbst bezeugt");
  assert.equal(new Set(gemeldet.map((g) => g.id)).size, 71, "keine Kennung doppelt");
...
test("Ein abstuerzendes Modul reisst den Lauf nicht mit", async () => {
  ...
  assert.equal(ergebnisse.length, 70, "alle anderen laufen trotzdem");
...
test("Mit Netz kommt der E2E-Waechter dazu — und meldet ehrlich, wenn er nicht pruefen kann", async () => {
  ...
  assert.equal(ergebnisse.length, 73, "E2E-Waechter, Voice-Region und Sync-Waechter sind die drei zusaetzlichen Netz-Laeufe (plus Nr. 65, die fuenf Deckungs-Waechter Nr. 66-70, Nr. 72 und die Schutz-Echtheit Nr. 82 ohne Netz)");
