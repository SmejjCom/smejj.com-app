...
test("Der Laeufer betreibt alle Selbsttest-Autopiloten und meldet jeden einzeln", async () => {
  ...
  assert.equal(ergebnisse.length, 31, "30 Autopiloten laufen ohne Netz im Control-Server (seit 2026-08-14 mit der AI Evolution Engine, Nr. 37-39)");
  assert.equal(gemeldet.length, 32, "30 Laeufe + der Taktgeber, der sich selbst bezeugt");
  assert.equal(new Set(gemeldet.map((g) => g.id)).size, 32, "keine Kennung doppelt");
...
test("Ein abstuerzendes Modul reisst den Lauf nicht mit", async () => {
  ...
  assert.equal(ergebnisse.length, 31, "alle anderen laufen trotzdem");
...
test("Mit Netz kommt der E2E-Waechter dazu — und meldet ehrlich, wenn er nicht pruefen kann", async () => {
  ...
  assert.equal(ergebnisse.length, 33, "E2E-Waechter und Voice-Region sind die beiden Netz-Laeufe");
