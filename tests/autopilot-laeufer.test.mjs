...
test("Der Laeufer betreibt alle 22 Selbsttest-Autopiloten und meldet jeden einzeln", async () => {
  const gemeldet = [];
  const ergebnisse = await laufeAlle({ melde: (id, e) => { gemeldet.push({ id, ...e }); return true; }, mitNetz: false });

  assert.equal(ergebnisse.length, 22, "22 Autopiloten laufen ohne Netz im Control-Server");
  assert.equal(gemeldet.length, 22, "jeder bekommt genau einen Herzschlag");
  assert.equal(new Set(gemeldet.map((g) => g.id)).size, 22, "keine Kennung doppelt");
...
test("Ein abstuerzendes Modul reisst den Lauf nicht mit", async () => {
  ...
  assert.equal(ergebnisse.length, 22, "alle anderen laufen trotzdem");
...
test("Mit Netz kommt der E2E-Waechter dazu — und meldet ehrlich, wenn er nicht pruefen kann", async () => {
  ...
  assert.equal(ergebnisse.length, 24, "E2E-Waechter und Voice-Region sind die beiden Netz-Laeufe");
