...
test("master policy locks GitHub and Cloudflare to permanent free-only roles", () => {
  assert.match(masterPolicy, /GitHub\.com darf nur im dauerhaft kostenlosen Free-Tarif genutzt werden/);
  assert.match(masterPolicy, /Cloudflare\.com darf nur im dauerhaft kostenlosen Free-Tarif genutzt werden/);
...
test("README points to the master policy and does not recommend paid core paths", () => {
  ...
  assert.match(readme, /Cloudflare\.com bleibt dauerhaft Free-only/);
...
test("platform constants state the free-only storage policy", () => {
  assert.match(platform, /GitHub Free and Cloudflare Free only; IDrive e2 is primary storage/);
...
test("local agent rules load the free-only policy for future work", () => {
  ...
  assert.match(agents, /Cloudflare\.com darf nur im dauerhaft kostenlosen Free-Tarif genutzt werden/);
