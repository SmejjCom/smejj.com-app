...
test("Nr. 75/79: neue Brücken-Version macht das alte Urteil ungültig, der Verstoss traegt einen Beleg (Master-Audit 15.09.)", async () => {
  ...
  // Geänderter Fall (z. B. belegtes Synonym in der Zusicherung) → neu messen.
  const geaendert = { ...gut, assertions: [{ type: "contains_any", values: ["smejj.com", "smejj"], critical: true }] };
  const nachFall = await messlaufImTakt({ kennung: "ver-1", faelleLader: async () => [geaendert], ablage: a, env, fetchImpl, versionLeser, sleep: async () => {} });
  assert.match(nachFall.meldung, /Messung gestartet/, "neue Fälle = neu messen");
  await warteAufMessung("ver-1");
  assert.equal(a.m.get(ABLAGE_ID).faelleHash, faelleHash([geaendert]));
  // Version unbekannt (Brücke antwortet nicht): kein Dauer-Nachmessen.
  version = null;
  const still = await messlaufImTakt({ kennung: "ver-1", faelleLader: async () => [geaendert], ablage: a, env, fetchImpl, versionLeser, sleep: async () => {} });
