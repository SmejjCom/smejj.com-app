...
# docs/prompts/AUFTRAG_TRAINING_24_7.md — also der Auftrag selbst,
# ohne die Erklaerung, wofuer die Datei da ist. Genau das, was man in eine neue
# Sitzung einfuegt.
set -euo pipefail

APP_VERZ="${0:A:h}"
QUELLE="$APP_VERZ/docs/prompts/AUFTRAG_TRAINING_24_7.md"

if [ ! -f "$QUELLE" ]; then
  osascript -e 'display alert "Auftrag nicht gefunden" message "docs/prompts/AUFTRAG_TRAINING_24_7.md fehlt." as critical'
  exit 1
fi
...
osascript -e "display dialog \"Der Auftrag liegt in der Zwischenablage — $ZEILEN Zeilen.\n\nJetzt eine neue Sitzung oeffnen und mit Cmd+V einfuegen.\n\nDer Text steht fuer sich allein: er enthaelt die gemessenen Zahlen, die betroffenen Dateien, die harten Grenzen und die Abnahmekriterien.\" buttons {\"Verstanden\"} default button 1 with title \"smejj.com — Auftrag Dauertraining\""
