#!/bin/zsh
# Einmal-Kaskade 21.09.2026 (dritte Runde) — Landeseite ohne Preise ausliefern.
#
...
WT_BASIS=$(git rev-parse "${BASIS_UEBERSCHREIBEN:-6a87fbcf}")  # der Stand, der als v937 live ging
...
SCHON=0
if [ -d "$BAU" ] && git -C "$BAU" log -1 --pretty=%s 2>/dev/null | grep -q "LANDESEITE zeigte die Preise"; then
  SCHON=1
...
ANKER="schutz-100-2026-09-21-landeseite-${SW_NEU#smejj-shell-}"
...
if [ "$GESTEMPELT" = "1" ]; then
  ...
  git "${autor[@]}" commit -q -m "chore(lock): Sperren nach dem Preis-Ausblenden der Landeseite gestempelt ($SW_NEU)" || true
fi
...
git "${autor[@]}" commit -q -m "deploy(ios): Landeseite ohne Preise in der iOS-Huelle (Apple 3.1.1); $SW_NEU — Quelle ${WT_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
...
C=$(curl -s -o /dev/null -w "%{http_code}" -m 20 "https://smejj.com/assets/ios-preise-aus.js?n=$RANDOM")
[ "$C" = "200" ] && echo "  ios-preise-aus.js ist da" || { echo "  ios-preise-aus.js FEHLT ($C)"; OK=0; }
# Die Landeseite muss die Weiche SYNCHRON laden und alle drei Stellen markieren.
LS=$(curl -s -m 20 "https://smejj.com/willkommen.html?n=$RANDOM")
echo "$LS" | grep -q 'ios-preise-aus.js' && echo "  Landeseite laedt die Weiche" || { echo "  Landeseite OHNE Weiche"; OK=0; }
echo "$LS" | grep 'ios-preise-aus.js' | grep -qE 'defer|async|type="module"' && { echo "  Weiche ist aufgeschoben — Preise wuerden aufblitzen"; OK=0; } || echo "  Weiche laeuft synchron"
N=$(echo "$LS" | grep -c 'data-nur-web')
[ "$N" -ge 3 ] && echo "  $N Stellen markiert (Navigation, Knopf, Abschnitt)" || { echo "  nur $N Stellen markiert — erwartet 3"; OK=0; }
echo "$LS" | grep -q 'html\[data-huelle="ios"\] \[data-nur-web\]' && echo "  CSS-Regel steht in der Seite" || { echo "  CSS-Regel FEHLT"; OK=0; }
