#!/bin/zsh
# smejj.com — Einmal-Kaskade fuer den Betreiber-Doppelklick (2026-09-07, Runde 3):
# Touch-Ziele 44 px auf Tablets und im Querformat, Rest-Ziele am Handy —
# Start-Lock stempeln und sw.js (Sprung auf v798) ausliefern.
#
...
#   2. kompakt.js, mobil-dock.js und Tests aus dem QA-Zweig; sw.js = LIVE (v797) -> v798
#   3. Tests und Waechter
#   4. Sicherheitsnetz: live ist v797, kompakt.js live = Bauzweig-Basis
...
BAUM="/private/tmp/claude-501/stempel-mobil-ziele3-$(date +%Y%m%d-%H%M%S)"
SW_VORHER="smejj-shell-v797"
SW_NEU="smejj-shell-v798"
WORTLAUT="Betreiber Wof Kadavanich, 2026-09-07 (Auftrag 100 % End-to-End Responsiveness, 'Teste du selber, weiter'): Touch-Ziele mindestens 44x44 auf Tablets und im Querformat (kompakt.js, pointer:coarse) sowie Banner, Profilbild-Knopf und Werkzeug-Zeilen am Handy (mobil-dock.js); sw.js CACHE_NAME smejj-shell-v798 (Runde 3: Schreibfeld-Knoepfe, Code-Leiste, Banner auf Tablets 44 px). Stempel per Doppelklick."
...
git -C "$KLON" show origin/main:sw.js > /tmp/sw-live-ziele3.js || { echo "ABBRUCH: Live-sw.js nicht lesbar"; behalten 5; }
grep -q "const CACHE_NAME = \"$SW_VORHER\";" /tmp/sw-live-ziele3.js || { echo "ABBRUCH: Live-sw.js traegt nicht $SW_VORHER — Basis stimmt nicht mehr"; behalten 5; }
grep -q '"/assets/mobil-dock.js"' /tmp/sw-live-ziele3.js || { echo "ABBRUCH: Live-sw.js kennt mobil-dock.js nicht (Runde 1 fehlt?)"; behalten 5; }
sed "s#const CACHE_NAME = \"$SW_VORHER\";#const CACHE_NAME = \"$SW_NEU\";#" /tmp/sw-live-ziele3.js > /tmp/sw-neu-ziele3.js
grep -q "$SW_NEU" /tmp/sw-neu-ziele3.js || { echo "ABBRUCH: Version nicht gesetzt"; behalten 5; }
cp /tmp/sw-neu-ziele3.js public/sw.js
npm run -s build:assets >/dev/null 2>&1 || true
grep -q "Runde 3" public/kompakt.js || { echo "ABBRUCH: kompakt.js traegt die Touch-Regel nicht"; behalten 5; }
git status --short | sed 's/^/    /' | head -20

echo "3/8 Tests und Waechter ..."
if ! node --test tests/mobil-dock.test.mjs tests/touch-ziele.test.mjs tests/modell-router.test.mjs tests/pwa-vollbild-heilung.test.mjs tests/composer-zeile.test.mjs tests/precache-dynamische-importe.test.mjs tests/platform-pwa.test.mjs > /tmp/mobil-ziele3-kaskade.log 2>&1; then
  echo "ABBRUCH: Tests rot"; grep -E "not ok|✖|Error" /tmp/mobil-ziele3-kaskade.log | head -20; behalten 6
fi
grep -E "ℹ (pass|fail)" /tmp/mobil-ziele3-kaskade.log | tr '\n' ' '; echo
...
git -c user.name="Wof Kadavanich" -c user.email="smejjcom@gmail.com" commit -q \
  -m "fix(mobil)+chore(start-lock): Touch-Ziele Runde 3: Schreibfeld-Knoepfe, Code-Leiste, Banner auf Tablets 44 px (kompakt.js); sw.js $SW_NEU — Stempel per Betreiber-Doppelklick 2026-09-07" \
...
cp /tmp/sw-neu-ziele3.js sw.js; cp /tmp/sw-neu-ziele3.js assets/sw.js
git add kompakt.js assets/kompakt.js mobil-dock.js assets/mobil-dock.js sw.js assets/sw.js
git -c user.name="Wof Kadavanich" -c user.email="smejjcom@gmail.com" commit -q \
  -m "deploy(mobil): Touch-Ziele Runde 3 (kompakt.js); SW $SW_NEU — Quelle smejj.com-app $QUELLKENNUNG (Stempel 2026-09-07)" \
