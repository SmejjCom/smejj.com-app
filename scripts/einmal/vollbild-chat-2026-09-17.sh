#!/bin/zsh
# smejj.com — Kaskade 2026-09-17: Vollbild-Chat am Handy (iPhone-PWA) und Diktat-Fix ausliefern.
#
# Betreiber-Auftrag 17.09. (schriftlich im Chat, mit iPhone-Screenshots): "Bitte den mobilen
# Chat-Bereich – besonders die aus Chrome installierte PWA auf iPhone – wie bei ChatGPT als
# echtes Edge-to-Edge-Fullscreen optimieren." Fuenf Punkte: Statusleiste transparent ueber dem
# Chat, Logo/Globus hoeher, Schreibfeld schwebend, Diktat Start/Stop beliebig oft, Lesbarkeit.
#
# Was ausgeliefert wird (SW v896 -> v897):
#   design-v14-vollbild-chat.css (neu, letztes Kaskaden-Ende im Buendel start-styles.css)
#   vollbild-chat.js (neu, misst das schwebende Feld), mobil-dock.js (laedt es nach)
#   composer-dictation.js (Umbau: frische Erkennung je Start, Ergebnisliste neu aufgebaut)
#   index.html (Stil-Marke v14a), sw.js (v897, Precache-Eintrag)
# Gemessen im iPhone-17-Pro-Simulator als installierter Webclip: Bericht docs/qa/vollbild-chat-ios-2026-09-17.md
#
...
BAU="/private/tmp/claude-501/bau-vollbild"
...
SW_VORHER="smejj-shell-v896"
SW_NEU="smejj-shell-v897"
WORTLAUT="Betreiber 17.09.2026 schriftlich im Chat: 'Bitte den mobilen Chat-Bereich – besonders die aus Chrome installierte PWA auf iPhone – wie bei ChatGPT als echtes Edge-to-Edge-Fullscreen optimieren.' Punkte 1-5: Fullscreen/Statusleiste transparent, Logo+Browser-Icon hoeher, Schreibfeld schwebend, Diktier-Mikrofon Start/Stop beliebig oft, Lesbarkeit. 'Nicht nur optisch simulieren … auf echter iOS-PWA/iPhone vollstaendig testen.' Umsetzung: design-v14-vollbild-chat.css, vollbild-chat.js, composer-dictation.js, SW smejj-shell-v897. Per Doppelklick ausgeloest."
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock fuer Vollbild-Chat am Handy gestempelt (SW v897)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
  ...
  node --test tests/composer-dictation.test.mjs tests/kopfglas-verdeckt-nicht.test.mjs tests/precache-dynamische-importe.test.mjs >/dev/null 2>&1 || { echo "ABBRUCH: Tests rot ($d)."; exit 1; }
...
if [ "$LIVE_SW" = "$SW_NEU" ]; then
  ...
  git "${autor[@]}" commit -q -m "deploy(vollbild): Chat am Handy als Vollbild — Statusleiste transparent, Feld schwebt, Diktat-Fix; SW $SW_NEU — Quelle smejj.com-app ${APP_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
