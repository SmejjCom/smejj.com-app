#!/bin/zsh
# Einmal-Kaskade 18.09.2026 — eingebauter Browser, A-bis-Z-Livetest (SW v901).
#
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock nach dem Browser-A-bis-Z-Livetest gestempelt (SW v901)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
...
curl -s -m 20 "https://smejj.com/assets/start-styles.css?n=$RANDOM" | grep -q "browser-pane-open #browserButton" && echo "  Buendel live traegt die Globus-Regel" || echo "  ABWEICHEND: Buendel live ohne Globus-Regel (Rand-Cache?)"
...
ANKER="schutz-100-2026-09-18-browser"
