...
BASIS_VOR_AENDERUNG="9aa86e6b"
CODE_COMMIT="e5143015"
SW_VORHER="smejj-shell-v865"
SW_NEU="smejj-shell-v866"
...
WT="/private/tmp/claude-501/kaskade-design-v12c"
...
grep -q 'start-styles.css?v=v12f-20260913' public/index.html || { echo "ABBRUCH: index.html traegt nicht die V12-Marke."; exit 1; }
...
if git diff --cached --quiet; then echo "(Manifest unveraendert)"; else
  git commit -q -m "chore(start-lock): Stempel Design V12 Nachzug 2 (Feld bis zur Unterkante, Werkzeug-Raster) 2026-09-13 — index.html, start-styles.css (V12 Spur/Chat/Code/Vollbild), spur-start.js, mobil-dock.js, SW $SW_NEU (Betreiber-Doppelklick)
...
git commit -q -m "deploy(design-v12c): Feld bis zur Unterkante (Inset im Feld), Werkzeuge 4x2 im Chat; zuvor Spur in drei Zonen, Chat/Start 19 px, Feld zwei Zeilen, Werkzeuge mit Wort, Code-Vorlagen sichtbar, Vollbild (Grund App-Farbe, Fehlbetrag-Ausgleich); SW $SW_NEU — Quelle smejj.com-app $QUELLE" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
