...
BASIS_VOR_AENDERUNG="0489c5a9"
CODE_COMMIT="2652e654"
SW_VORHER="smejj-shell-v866"
SW_NEU="smejj-shell-v867"
...
WT="/private/tmp/claude-501/kaskade-design-v12d"
...
grep -q 'start-styles.css?v=v12g-20260914' public/index.html || { echo "ABBRUCH: index.html traegt nicht die V12-Marke."; exit 1; }
...
if git diff --cached --quiet; then echo "(Manifest unveraendert)"; else
  git commit -q -m "chore(start-lock): Stempel Design V12 Nachzug 3 (Knoepfe an der Unterkante, Feld kompakt) 2026-09-13 — index.html, start-styles.css (V12 Spur/Chat/Code/Vollbild), spur-start.js, mobil-dock.js, SW $SW_NEU (Betreiber-Doppelklick)
...
git commit -q -m "deploy(design-v12d): Knoepfe an der Unterkante, Feld kompakt; zuvor Spur in drei Zonen, Chat/Start 19 px, Feld zwei Zeilen, Werkzeuge mit Wort, Code-Vorlagen sichtbar, Vollbild (Grund App-Farbe, Fehlbetrag-Ausgleich); SW $SW_NEU — Quelle smejj.com-app $QUELLE" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
