#!/bin/zsh
# smejj.com — Betreiber-Kaskade 2026-09-06 (Nacht, dritter Klick): Sperrseiten
# erkennen — stempeln und ausliefern.
#
# BEFUND (live duckduckgo.com, 23:10): DDG liefert dem fernen Browser die
# Sperrseite static-pages/418.html ("Unexpected error"); die Maus tippte und
# schickte zehnmal ab, bis der Not-Aus kam. Jetzt: Sperr-/Fehlerseiten
# (Roboter-Pruefung, Cloudflare, 403/418/429, "unusual traffic") aus der eigenen
# Beobachtung erkennen — beim ersten Mal erfaehrt es das Modell, beim zweiten
# Mal endet der Lauf mit klarem Grund und Hinweis auf die Maus-Bruecke.
# CODE: 3c76c35b (Arbeitszweig feature/design-v11), Service-Worker v788.
# SICHERHEITSNETZ: alle 10 Dateien im Live-Repo byte-gleich mit c115d6c2
# (= Stempel Maus-Schrittgrenze, live SW v787).
set -uo pipefail
REPO="/Users/alanbest/Library/CloudStorage/GoogleDrive-smejjcom@gmail.com/.shortcut-targets-by-id/1FZNCd1vuQbdTkRgF0Vtz8htM8e5JhPbY/- smejj.com info/smejj.com App"
KLON="/Users/alanbest/smejj-app-frontend"
BASIS_VOR_AENDERUNG="c115d6c2"
CODE_COMMIT="3c76c35b"
SW_VORHER="smejj-shell-v787"
SW_NEU="smejj-shell-v788"
...
grep -q 'erkenneSperrseite' public/browser-pane-maus.js || { echo "ABBRUCH: die Sperrseiten-Erkennung steht nicht in browser-pane-maus.js."; exit 1; }
...
node scripts/check-start-lock.mjs --freeze --confirm "Betreiber, 2026-09-06 23:15: Fehlerseiten erkennen (Empfehlung gewaehlt). Live-Befund duckduckgo.com: Sperrseite 418 zehnmal wiederholt. Umgesetzt: Sperr- und Fehlerseiten aus der eigenen Beobachtung erkennen (browser-pane-maus.js), einmal melden, beim zweiten Mal aufhoeren. Marken bis zur Wurzel (browser-pane-maus 20260906-7, browser-pane 20260906-10, maus-absicht v34, maus-panel v28, sendepfad-nachladen v18, browser-nachladen v16, app.js b154), Service-Worker smejj-shell-v788. Stempel per Doppelklick im Finder." \
...
if git diff --cached --quiet; then echo "(Manifest unveraendert)"; else
  git commit -q -m "chore(start-lock): Stempel Maus-Sperrseiten 2026-09-06 — app.js b154, index.html, SW smejj-shell-v788 (Betreiber-Doppelklick)
...
git commit -q -m "deploy(maus): Sperrseiten erkennen, einmal melden, dann aufhoeren; SW $SW_NEU — Quelle smejj.com-app $QUELLE" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
