#!/bin/zsh
# Einmal-Kaskade 18.09.2026, RUNDE 7 — Proxy-Seite komprimiert ausliefern (live 15-30 s Uebertragung fuer 305 KB), SW v909:
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock nach dem Browser-A-bis-Z-Livetest gestempelt (SW v909)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
...
ANKER="schutz-100-2026-09-20-browser-v909"
