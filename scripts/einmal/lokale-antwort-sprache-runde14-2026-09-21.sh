#!/bin/zsh
# Einmal-Kaskade 20.09.2026, RUNDE 14 — das Geraetemodell (Desktop-Chrome) antwortet in der Sprache der Frage, sein Hinweis ist uebersetzbar ($SW_NEU). BASIS = ausgelieferter Stand v944.
...
for d in "$APP" "$BAU"; do
  ...
  if [ "$ALT" != "$SW_NEU" ]; then
    ...
    git add public/sw.js public/assets/sw.js && git "${autor[@]}" commit -q -m "chore(sw): Cache-Nummer $SW_NEU (Geraetemodell-Sprache)" || { echo "ABBRUCH: Nummern-Commit $d."; exit 1; }
  fi
...
if [ "$LIVE_SW" = "$SW_NEU" ]; then
  ...
  git "${autor[@]}" commit -q -m "deploy(sprache): Geraetemodell antwortet in der Sprache der Frage, Hinweis in 14 Sprachen; SW $SW_NEU — Quelle smejj.com-app ${APP_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
