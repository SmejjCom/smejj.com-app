#!/bin/zsh
# Einmal-Kaskade 23.09.2026 — drei Geraetebefunde vom 22.09. behoben (smejj-shell-v960).
# Betreiber: "Die drei Geraetebefunde beheben, A bis Z." (1) Profilmenue-Lage ueber die
# Oberkante aus dem Anker statt bottom/innerHeight, (2) Textauswahl bei Ansichtswechsel
# aufheben, (3) Gastfrage mit Hinweis nach 15 s und Wache an der Uhrzeit.
...
BAU="$HOME/smejj-app-befunde-bau"
...
BASIS="e22f00c6"    # der Stand, der als smejj-shell-v959 live ging
WORTLAUT="Betreiber 23.09.2026 im Chat: 'Die drei Geraetebefunde beheben, A bis Z' (Befunde vom 22.09.: Profilmenue-Tipp geht durch, iOS-Auswahlleiste klebt, Gastfrage haengt). Geaendert: profile-dock-menu.js/profile-dock.css (Lage + Bedienflaeche), mobil-dock.js (Auswahl aufheben), gast-frage.js + willkommen-sprache.js + willkommen.html (Hinweis, Wache), Marke profile-dock.js -14, Buendel, Cache v960. Nichts sonst an der Startseite."
...
ANKER="schutz-100-2026-09-23-geraetebefunde-${SW_NEU#smejj-shell-}"
...
if [ "$GESTEMPELT" = "1" ]; then
  ...
  git "${autor[@]}" commit -q -m "chore(lock): Sperren nach den drei Geraetebefunden gestempelt ($SW_NEU)" || true
fi
...
git "${autor[@]}" commit -q -m "deploy(handy): drei Geraetebefunde behoben (Profilmenue, Auswahlleiste, Gastfrage); $SW_NEU — Quelle ${WT_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
...
echo "== 6. Nachweis"
OK=1
G=$(curl -s -m 20 "https://smejj.com/assets/gast-frage.js?v=3&n=$RANDOM")
echo "$G" | grep -q "HINWEIS_MS = 15000" && echo "  gast-frage.js live mit Hinweis-Frist" || { echo "  gast-frage.js OHNE Hinweis"; OK=0; }
echo "$G" | grep -q 'addEventListener("visibilitychange", wache)' && echo "  gast-frage.js live mit Uhrzeit-Wache" || { echo "  gast-frage.js OHNE Wache"; OK=0; }
P=$(curl -s -m 20 "https://smejj.com/assets/profile-dock-menu.js?v=b58&n=$RANDOM")
echo "$P" | grep -q "export function lageUeberKnopf" && echo "  profile-dock-menu.js live mit Anker-Lage" || { echo "  profile-dock-menu.js OHNE lageUeberKnopf"; OK=0; }
M=$(curl -s -m 20 "https://smejj.com/assets/mobil-dock.js?n=$RANDOM")
echo "$M" | grep -q "export function raeumeAuswahlAuf" && echo "  mobil-dock.js live hebt die Auswahl auf" || { echo "  mobil-dock.js OHNE Auswahl-Wache"; OK=0; }
W=$(curl -s -m 20 "https://smejj.com/willkommen.html?n=$RANDOM")
echo "$W" | grep -q "gast-frage.js?v=3" && echo "  willkommen.html laedt gast-frage v3" || { echo "  willkommen.html laedt NICHT v3"; OK=0; }
I=$(curl -s -m 20 "https://smejj.com/index.html?n=$RANDOM")
echo "$I" | grep -q "profile-dock-20260922-14" && echo "  index.html laedt profile-dock -14" || { echo "  index.html laedt NICHT -14"; OK=0; }
C=$(curl -s -m 20 "https://smejj.com/assets/start-styles.css?v=v16b-20260922&n=$RANDOM")
echo "$C" | grep -q "touch-action: manipulation;" && echo "  Buendel live mit Bedienflaechen-Regel" || { echo "  Buendel OHNE touch-action"; OK=0; }
...
if [ "$OK" = "1" ]; then
  ...
  echo "  am Geraet nachmessen: Menue -> Mein Konto oeffnet das Konto, auch nach Tastatur auf/zu."
else
