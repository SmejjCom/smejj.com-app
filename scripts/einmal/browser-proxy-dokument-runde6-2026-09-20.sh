#!/bin/zsh
# Einmal-Kaskade 18.09.2026, RUNDE 6 — Proxy-Seiten als eigenes Dokument (Stil, Bilder, Bedienung), https zuerst (SW v908):
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock nach dem Browser-A-bis-Z-Livetest gestempelt (SW v908)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
...
ANKER="schutz-100-2026-09-20-browser-v908"
