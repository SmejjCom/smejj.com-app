#!/bin/zsh
# Einmal-Kaskade 18.09.2026, RUNDE 2 — Befunde aus dem Live-Nachtest von v901 (SW v902):
# Escape gehoert dem offenen Menue; Fehlerseiten des Servers (Wikipedia 403) gehen in den echten Browser.
#
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock nach dem Browser-A-bis-Z-Livetest gestempelt (SW v902)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
  ...
  node --test tests/browser-livetest-2026-09-18.test.mjs tests/browser-pane.test.mjs tests/browser-pane-chrome-abgleich.test.mjs tests/csp-hosts.test.mjs tests/module-queries.test.mjs tests/precache-dynamische-importe.test.mjs >/dev/null 2>&1 || { echo "ABBRUCH: Tests rot ($d)."; exit 1; }
...
curl -s -m 20 "https://smejj.com/assets/start-styles.css?n=$RANDOM" | grep -q "browser-pane-open #browserButton" && echo "  Buendel live traegt die Globus-Regel" || echo "  (Buendel: Rand-Cache — Dateivergleich oben ist massgeblich)"
...
ANKER="schutz-100-2026-09-18-browser-v902"
