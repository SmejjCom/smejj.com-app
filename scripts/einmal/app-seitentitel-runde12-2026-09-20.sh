#!/bin/zsh
# Einmal-Kaskade 20.09.2026, RUNDE 12 — Seitentitel und Ueberschriften der Ansichten in 14 Sprachen (SW v917). BASIS = ausgelieferter Stand v916.
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock nach dem Browser-A-bis-Z-Livetest gestempelt (SW v917)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
  if node scripts/check-security-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/security/security-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Sicherheits-Lock nachgestempelt — /api/browser/page als offene Lese-Route, Sprachzeile der Chat-Bruecke (Betreiber 20.09.) (SW v917)" || { echo "ABBRUCH: Sicherheits-Stempel-Commit $d."; exit 1; }
  fi
  ...
  node --test tests/browser-livetest-2026-09-18.test.mjs tests/browser-nachladen.test.mjs tests/frage-erfassung-adresse.test.mjs tests/view-title-sprache.test.mjs tests/chat-menue-sprache.test.mjs tests/i18n-ui.test.mjs tests/profile-dock.test.mjs tests/inhalt-melden.test.mjs tests/chat-message-actions.test.mjs tests/chat-menue-mehr.test.mjs tests/precache-dynamische-importe.test.mjs tests/touch-ziele.test.mjs tests/remote-browser-session.test.mjs tests/browser-pane.test.mjs tests/browser-pane-chrome-abgleich.test.mjs tests/csp-hosts.test.mjs tests/module-queries.test.mjs tests/precache-dynamische-importe.test.mjs >/dev/null 2>&1 || { echo "ABBRUCH: Tests rot ($d)."; exit 1; }
...
if [ "$LIVE_SW" = "$SW_NEU" ]; then
  ...
  git "${autor[@]}" commit -q -m "deploy(i18n): Seitentitel und Ueberschriften der Ansichten in 14 Sprachen; SW $SW_NEU — Quelle smejj.com-app ${APP_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
...
ANKER="schutz-100-2026-09-20-app-v917"
