#!/bin/zsh
# Einmal-Kaskade 18.09.2026, RUNDE 3 — A-bis-Z-Test der ganzen App am 19.09. (SW v903):
# toter Browser-Knopf der Seitenleiste (Nachlader), 44-px-Kopfknoepfe am Handy, Radar-Tests nachgezogen.
#
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock nach dem Browser-A-bis-Z-Livetest gestempelt (SW v903)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
...
ANKER="schutz-100-2026-09-19-app-v903"
