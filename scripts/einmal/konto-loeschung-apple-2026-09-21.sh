#!/bin/zsh
# Einmal-Kaskade 21.09.2026 — Konto-Loeschung fuer jeden Anmeldeweg ausliefern.
#
# ANLASS: Apple hat die App nach Richtlinie 2.1 abgelehnt. Beim Aufarbeiten fiel
# das naechste Risiko auf: Richtlinie 5.1.1(v) verlangt, dass eine Kontoloeschung
# IN der App startet. Bei uns ging das nur fuer E-Mail-Konten — Google, GitHub
# und Passkey sind passwortlos, der Server verlangte aber ein Passwort und
# antwortete `account_delete_requires_email_login`. Der Betreiber hat den Umbau
# am 21.09.2026 schriftlich freigegeben ("Ja, jetzt umbauen").
#
...
BAU="$HOME/smejj-konto-loeschung-bau"
...
WORTLAUT="Betreiber 21.09.2026 schriftlich auf die Auswahlkarte: 'Ja, jetzt umbauen' — Konto-Loeschung fuer Google-, GitHub- und Passkey-Konten in der App (Apple Richtlinie 5.1.1(v)); das Passwortfeld entfaellt dort, die woertliche Bestaetigung bleibt Pflicht und gilt jetzt in beiden Sprachfassungen."
...
SCHON=0
if [ -d "$BAU" ] && git -C "$BAU" log -1 --pretty=%s 2>/dev/null | grep -q "jeden Anmeldeweg"; then
  SCHON=1
...
ANKER="schutz-100-2026-09-21-kontoloeschung-${SW_NEU#smejj-shell-}"
...
if [ "$GESTEMPELT" = "1" ]; then
  ...
  git "${autor[@]}" commit -q -m "chore(lock): Sperren nach der Konto-Loeschung fuer jeden Anmeldeweg gestempelt ($SW_NEU)" || true
fi
...
git "${autor[@]}" commit -q -m "deploy(konto): Loeschung fuer jeden Anmeldeweg (Apple 5.1.1(v)); $SW_NEU — Quelle ${WT_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
...
K=$(curl -s -m 20 "https://smejj.com/assets/account-sessions.js?n=$RANDOM" | grep -c 'DELETE ACCOUNT')
[ "$K" -ge 1 ] && echo "  account-sessions.js traegt das englische Loeschwort" || { echo "  account-sessions.js OHNE das englische Loeschwort"; OK=0; }
E=$(curl -s -m 20 "https://smejj.com/assets/i18n/en.js?n=$RANDOM" | grep -c 'Delete account permanently')
[ "$E" -ge 1 ] && echo "  en.js traegt die Loesch-Schluessel" || { echo "  en.js OHNE die Loesch-Schluessel"; OK=0; }
# Die englischen Schluessel liegen in en-2.js und werden ueber en.js nachgeladen.
Z=$(curl -s -m 20 "https://smejj.com/assets/i18n/en-2.js?n=$RANDOM" | grep -c 'Delete account permanently')
[ "$Z" -ge 1 ] && echo "  en-2.js traegt die Loesch-Schluessel" || echo "  (Loesch-Schluessel stehen in en.js selbst)"
