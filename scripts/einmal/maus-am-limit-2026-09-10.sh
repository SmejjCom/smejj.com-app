...
node scripts/check-start-lock.mjs --freeze --confirm "Betreiber, 2026-09-10: 'Mach komplett 100 % fertig, lass nichts offen.' Umgesetzt: erschoepftes Modell-Kontingent wird im Chat als solches benannt (planer_am_limit + Wartezeit) statt als Ausfall, Marken bis zur Wurzel, Stempel aus der Sitzung per Kaskade." \
...
node "$WT/scripts/einmal/maus-am-limit-2026-09-10.mjs" "$KLON" || { git checkout -q -- . ; echo "ABBRUCH: Fix nicht angewendet, Klon zurueckgesetzt."; exit 1; }
SW_NEU=$(cat /tmp/maus-am-limit-sw.txt)
git add -A && git status --short | head -20
git commit -q -m "deploy(maus): erschoepftes Kontingent heisst im Chat 'alle Modelle sind gerade am Limit'; SW $SW_NEU — Quelle smejj.com-app feature/design-v11 (auf den Live-Stand gelegt)" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
