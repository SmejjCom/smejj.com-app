#!/usr/bin/env node
// smejj.com — "nth" im Panel AUF DEN LIVE-STAND legen (2026-09-09).
//
// WARUM SO: Live ist die Maus dreigeteilt (browser-pane-maus-plan.js /
// -frei.js), der Arbeitszweig traegt den Monolithen. Ein Deploy aus dem
// Zweig wuerde fremde Live-Arbeit ueberschreiben. Darum: ZWEI Textstellen
// auf die Live-Dateien legen, Marken der Importeure hochzaehlen, SW +1.
//   1. browser-pane-maus-plan.js: selektorAus reicht "nth" durch.
//   2. browser-pane-maus-frei.js: Fehlertexte 220 statt 120 Zeichen — der Rat
//      des Browsers ("nth":0 …) wurde sonst abgeschnitten.
// SICHERHEITSNETZ: beide Dateien muessen live exakt dem Stand entsprechen,
// gegen den der Fix gebaut wurde (sha256, 12 Zeichen) — sonst nichts geschrieben.
import { readFileSync, writeFileSync, existsSync, readdirSync } from "node:fs";
import { createHash } from "node:crypto";
import { join } from "node:path";

const [klon] = process.argv.slice(2);
if (!klon) { console.error("Aufruf: <klon>"); process.exit(2); }
const VORHER = { "browser-pane-maus-plan.js": "317ec8ab487d", "browser-pane-maus-frei.js": "51bf4a4aa05e" };
const sha = (text) => createHash("sha256").update(text).digest("hex").slice(0, 12);
const lies = (p) => readFileSync(p, "utf8");

for (const f of Object.keys(VORHER)) {
  const live = lies(join(klon, f));
  if (sha(live) !== VORHER[f]) { console.error(`ABBRUCH: ${f} live (${sha(live)}) ist nicht der erwartete Stand (${VORHER[f]}).`); process.exit(1); }
}
console.log("Sicherheitsnetz: beide Live-Dateien wie erwartet.");

const geaendert = new Set();
// 1. plan.js
{
  const p = join(klon, "browser-pane-maus-plan.js");
  const alt = `    ...(ziel.name !== undefined ? { name: ziel.name } : {})\n  };`;
  const neu = `    ...(ziel.name !== undefined ? { name: ziel.name } : {}),\n    // "nth" ist die BENANNTE Wahl bei gleichnamigen Treffern (Schema seit\n    // 21.08.). Bis 09.09. fiel es hier weg — live stoppte deshalb ein Lauf an\n    // zwei gleichen Wikipedia-Links, obwohl das Modell haette waehlen koennen.\n    ...(Number.isInteger(ziel.nth) && ziel.nth >= 0 ? { nth: ziel.nth } : {})\n  };`;
  const t = lies(p);
  if (t.split(alt).length !== 2) { console.error("ABBRUCH: selektorAus-Stelle nicht genau einmal gefunden."); process.exit(1); }
  writeFileSync(p, t.replace(alt, neu)); geaendert.add("browser-pane-maus-plan.js");
}
// 2. frei.js
{
  const p = join(klon, "browser-pane-maus-frei.js");
  let t = lies(p);
  const a = "String(blick.error).slice(0, 120)", b = "String(ergebnis.error).slice(0, 120)";
  if (t.split(a).length !== 2 || t.split(b).length !== 2) { console.error("ABBRUCH: Kuerzungs-Stellen nicht genau einmal gefunden."); process.exit(1); }
  t = t.replace(a, "String(blick.error).slice(0, 220)").replace(b, "String(ergebnis.error).slice(0, 220)");
  writeFileSync(p, t); geaendert.add("browser-pane-maus-frei.js");
}
// 3. Marken der Importeure im Live-Stand hochzaehlen — bis nichts mehr wandert.
const kandidaten = readdirSync(klon).filter((n) => n.endsWith(".js") || n === "index.html");
const bump = (m) => {
  const x = /^(.*-)(\d+)$/.exec(m); if (x) return x[1] + (Number(x[2]) + 1);
  if (/^\d+$/.test(m)) return String(Number(m) + 1);
  const b = /^b(\d+)$/.exec(m); if (b) return "b" + (Number(b[1]) + 1);
  throw new Error("unbekannte Marke " + m);
};
const offen = [...geaendert]; const protokoll = [];
while (offen.length) {
  const modul = offen.shift();
  const muster = new RegExp(modul.replace(/[.*+?^${}()|[\]\\]/g, "\\$&") + "\\?v=([A-Za-z0-9-]+)", "g");
  for (const g of kandidaten) {
    const pfad = join(klon, g); const text = lies(pfad);
    if (!muster.test(text)) continue;
    muster.lastIndex = 0;
    writeFileSync(pfad, text.replace(muster, (_, marke) => `${modul}?v=${bump(marke)}`));
    protokoll.push(`${g}: ${modul} Marke gehoben`);
    if (!geaendert.has(g) && g !== "index.html") { geaendert.add(g); offen.push(g); }
    if (g === "index.html") geaendert.add(g);
  }
}
// 4. Service-Worker +1
const swPfad = join(klon, "sw.js"); const sw = lies(swPfad);
const m = /const CACHE_NAME = "smejj-shell-v(\d+)";/.exec(sw);
if (!m) { console.error("ABBRUCH: CACHE_NAME nicht gefunden."); process.exit(1); }
const swNeu = `smejj-shell-v${Number(m[1]) + 1}`;
writeFileSync(swPfad, sw.replace(m[0], `const CACHE_NAME = "${swNeu}";`)); geaendert.add("sw.js");
// 5. assets/-Kopien
for (const f of geaendert) { const k = join(klon, "assets", f); if (existsSync(k)) writeFileSync(k, lies(join(klon, f))); }
console.log(protokoll.join("\n")); console.log(`SW: ${m[0].match(/v\d+/)[0]} -> ${swNeu}`);
console.log("GEAENDERT:", [...geaendert].sort().join(" "));
writeFileSync("/tmp/maus-nth-live-sw.txt", swNeu);
