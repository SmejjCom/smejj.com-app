#!/bin/zsh
# smejj.com — Betreiber-Kaskade 2026-09-03 (9): Vollbild am Handy, schwarzer Streifen unten weg.
#
...
FREIGABE="Betreiber Wof Kadavanich, 2026-09-03 (Doppelklick): Vollbild am Handy — Glas-Gefaelle auf dem Wurzelelement (html:root), kein schwarzer Streifen ueber dem Home-Balken; mobil-composer.css, Buendel-Marke mobil102-20260903 in index.html, Service-Worker-Cache +1. Grundlage: Auftrag 2026-09-03 'Vollbild, gesamte Flaeche' und 'alle Rechte, 100 % fertig'."
...
node --test tests/touch-ziele-waechter.test.mjs tests/frontend-structure.test.mjs tests/assets-sync.test.mjs tests/csp-hosts.test.mjs 2>&1 | grep -E "^ℹ (pass|fail)" | tr '\n' ' '; echo

echo "== 3. Commit design-v11"
GEAENDERT=$(git diff --name-only -- public docs/frontend tests | tr '\n' ' ')
git add ${=GEAENDERT}
git commit -q -m "fix(mobil): Vollbild — Glas-Gefaelle auf html:root, kein schwarzer Streifen ueber dem Home-Balken (iOS rechnet 100dvh kleiner als den Schirm); Buendel-Marke mobil102-20260903, SW smejj-shell-v${NEXT}, Start-Lock gestempelt (Betreiber-Doppelklick 2026-09-03)"
