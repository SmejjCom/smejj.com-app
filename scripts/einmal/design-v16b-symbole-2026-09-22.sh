#!/bin/zsh
# Einmal-Kaskade 22.09.2026 (Runde b) — Design V16b: die Symbole des Schreibfelds sitzen auf der Unterkante.
# Betreiber-Nachfrage: "Ja, Symbole auch auf die Kante setzen." Gemessen: Glyphen 10 px ueber dem
# Knopfrand (Grid zentriert) -> align-items: end, Tippziel bleibt 44 px.
#
...
BASIS="a3cda61e"    # der Stand, der als smejj-shell-v958 live ging
WORTLAUT="Betreiber 22.09.2026 im Chat: 'Ja, Symbole auch auf die Kante setzen' (Nachfrage zu 'Soll Schreibfeld ... mit ganz untere kante treffen'). Design V16b: nur die Ausrichtung der Symbole in den Knoepfen des Schreibfelds (align-items end), Buendel, index.html-Version und Cache-Nummer v959. Nichts sonst an der Startseite."
...
ANKER="schutz-100-2026-09-22-design-v16b-${SW_NEU#smejj-shell-}"
...
if [ "$GESTEMPELT" = "1" ]; then
  ...
  git "${autor[@]}" commit -q -m "chore(lock): Sperren nach Design V16b (Symbole auf der Unterkante) gestempelt ($SW_NEU)" || true
fi
...
git "${autor[@]}" commit -q -m "deploy(design): V16b — Symbole des Schreibfelds auf der Unterkante; $SW_NEU — Quelle ${WT_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
...
echo "$C" | grep -q "align-content: end" && echo "  V16b-Ausrichtung live" || { echo "  V16b-Ausrichtung fehlt"; OK=0; }
I=$(curl -s -m 20 "https://smejj.com/index.html?n=$RANDOM")
echo "$I" | grep -q "start-styles.css?v=v16b-20260922" && echo "  index.html zeigt auf v16b" || { echo "  index.html zeigt NICHT auf v16b"; OK=0; }
A=$(curl -s -m 20 "https://smejj.com/assets/index.html?n=$RANDOM")
echo "$A" | grep -q "start-styles.css?v=v16b-20260922" && echo "  assets/index.html zeigt auf v16b" || { echo "  assets/index.html zeigt NICHT auf v16b"; OK=0; }
...
if [ "$OK" = "1" ]; then
  ...
  echo "  am Geraet nachmessen: Glyphen der Symbole auf der Unterkante."
else
