#!/bin/zsh
# Einmal-Kaskade 21.09.2026 (vierte Runde) — Plan-Karte ohne Preis ausliefern.
#
...
WT_BASIS=$(git rev-parse "${BASIS_UEBERSCHREIBEN:-2762116b}")  # der Stand, der als v938 live ging
...
SCHON=0
if [ -d "$BAU" ] && git -C "$BAU" log -1 --pretty=%s 2>/dev/null | grep -q "Plan-KARTE nannte den Preis"; then
  SCHON=1
...
ANKER="schutz-100-2026-09-21-plankarte-${SW_NEU#smejj-shell-}"
...
if [ "$GESTEMPELT" = "1" ]; then
  ...
  git "${autor[@]}" commit -q -m "chore(lock): Sperren nach dem Preis-Ausblenden der Plan-Karte gestempelt ($SW_NEU)" || true
fi
...
git "${autor[@]}" commit -q -m "deploy(ios): Plan-Karte und Profilmenue ohne Preis (Apple 3.1.1); $SW_NEU — Quelle ${WT_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
...
echo "$AP" | grep -q 'planNameAnzeige' && echo "  Plan-Name laeuft ueber die Weiche" || { echo "  Plan-Name OHNE Weiche"; OK=0; }
echo "$AP" | grep -q 'iosHuelle() ? "Free" : "Free — 0 €"' && echo "  Free-Karte ohne Betrag in der Huelle" || { echo "  Free-Karte zeigt den Betrag"; OK=0; }
echo "$AP" | grep -q 'In dieser App wird nichts verkauft' && echo "  der Satz behauptet keinen Abo-Status" || { echo "  alter Satz noch da"; OK=0; }
DM=$(curl -s -m 20 "https://smejj.com/assets/profile-dock-menu.js?n=$RANDOM")
echo "$DM" | grep -q '!huelle' && echo "  Profilmenue ohne Betrag in der Huelle" || { echo "  Profilmenue zeigt den Betrag"; OK=0; }
echo "$DM" | grep -q '/profile#billing' && echo "  Mein Plan oeffnet den Plan-Reiter" || { echo "  Mein Plan oeffnet den falschen Reiter"; OK=0; }
