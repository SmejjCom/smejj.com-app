#!/bin/zsh
# smejj.com — Kaskade 2026-09-17: Medien-System A-Z ausliefern.
#
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock fuer Medien-System gestempelt (SW v894)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
...
for i in $(seq 1 80); do
  S=$(curl -s -o /dev/null -w "%{http_code}" -m 20 "https://api.smejj.com/medium/probe-$RANDOM")
  if [ "$S" = "403" ]; then echo "LIVE api.smejj.com: Medien-Routen antworten (403 fuer falschen Token) nach $((i*15)) s"; API_LIVE=1; break; fi
  echo "  api noch alt ($S) ..."
  sleep 15
done
...
if [ "$LIVE_SW" = "$SW_NEU" ]; then
  ...
  git "${autor[@]}" commit -q -m "deploy(medien): Medien-System A-Z — signierte Anzeige-Adressen, Vorschaubilder, Vollbild, Teilen als Datei oder widerrufbarer Link; SW $SW_NEU — Quelle smejj.com-app ${APP_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
