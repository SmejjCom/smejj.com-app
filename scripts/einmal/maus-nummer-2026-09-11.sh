#!/bin/zsh
# smejj.com — Kaskade 2026-09-10 (Abend): "Treffer n" in der Fortschrittszeile
...
node --test tests/browser-pane-maus.test.mjs tests/maus-nth-ausdruecklich.test.mjs tests/maus-entscheidung-reparatur.test.mjs tests/maus-schritt-nachfrage.test.mjs tests/maus-engine-route.test.mjs > /tmp/maus-am-limit-tests.log 2>&1 \
  || { echo "ABBRUCH: Tests rot."; tail -20 /tmp/maus-am-limit-tests.log; exit 1; }
grep -E "^ℹ (pass|fail)" /tmp/maus-am-limit-tests.log | tr '\n' ' '; echo
echo "== 2. Start-Lock stempeln (Betreiber-Wortlaut)"
node scripts/check-start-lock.mjs --freeze --confirm "Betreiber, 2026-09-11: 'Teste die gesamte App von A bis Z' + 'Maus zuverlaessig machen'. Umgesetzt: die Maus greift Elemente bei ihrer Nummer aus der Beobachtung (Panel reicht n durch, ferner Browser loest es auf) — gegen erfundene Selektoren, die in fuenf Messlaeufen 7 von 10 Fehlschlaegen ausmachten. Marken bis zur Wurzel, Stempel aus der Sitzung per Kaskade." \
...
git commit -q -m "fix(maus): Fortschrittszeile nennt die benannte Wahl (Treffer n); Server benennt dieselbe Wahl nach selector_mehrdeutig (nth 0) und entfernt Anfuehrungszeichen um Selektor-Werte; Marken bis zur Wurzel, Start-Lock gestempelt; Kaskade maus-am-limit
...
node "$WT/scripts/einmal/maus-nummer-2026-09-11.mjs" "$KLON" || { git checkout -q -- . ; echo "ABBRUCH: Fix nicht angewendet, Klon zurueckgesetzt."; exit 1; }
SW_NEU=$(cat /tmp/maus-nummer-sw.txt)
git add -A && git status --short | head -20
git commit -q -m "deploy(maus): die Maus greift Elemente bei ihrer Nummer statt geratene Selektoren; SW $SW_NEU — Quelle smejj.com-app feature/design-v11 (auf den Live-Stand gelegt)" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
