#!/bin/zsh
# smejj.com — Betreiber-Kaskade 2026-09-07: Browser-Oberflaeche nach dem
# Bildschirmfoto des Betreibers — stempeln und ausliefern.
#
# BEFUNDE: (1) Die Zeile "Live-Browser verbunden — …" nahm dauerhaft Platz ueber
# der Seite ein; jetzt schwebt ein Hinweis nur bei Fehlern und blendet nach
# 6 s aus, die Seite beginnt direkt unter der Adresszeile. (2) Drei "Zeiger":
# weisser Maus-Zeiger, tuerkiser Wiedergabe-Knopf neben dem Globus, Pfeil-Knopf
# in der Tableiste. Jetzt EIN Zeiger in Logo-Tuerkis (#02fdfd, Form wie
# icons/maus-zeiger.svg), Maus-Knopf als Geraete-Symbol, Wiedergabe-Knopf weg.
# CODE: 64c53e92 + Test-Nachzug (Arbeitszweig feature/design-v11), SW v789.
# SICHERHEITSNETZ: alle 14 Dateien im Live-Repo byte-gleich mit 8ae0c9ec
# (= Stempel Maus-Sperrseiten, live SW v788).
set -uo pipefail
REPO="/Users/alanbest/Library/CloudStorage/GoogleDrive-smejjcom@gmail.com/.shortcut-targets-by-id/1FZNCd1vuQbdTkRgF0Vtz8htM8e5JhPbY/- smejj.com info/smejj.com App"
KLON="/Users/alanbest/smejj-app-frontend"
BASIS_VOR_AENDERUNG="8ae0c9ec"
CODE_COMMIT="2214aab0"
SW_VORHER="smejj-shell-v788"
SW_NEU="smejj-shell-v789"
DATEIEN=(browser-stage.js browser-pane.css browser-pane.js browser-pane-render.js browser-pane-fernwege.js browser-pane-persistenz.js
  start-styles.css maus-absicht.js maus-panel.js sendepfad-nachladen.js browser-nachladen.js app.js index.html sw.js)
...
grep -q 'fill="#02fdfd"' public/browser-stage.js || { echo "ABBRUCH: der tuerkise Zeiger steht nicht in browser-stage.js."; exit 1; }
...
node --test tests/browser-stage.test.mjs tests/browser-pane.test.mjs tests/browser-pane-maus.test.mjs tests/browser-pane-chrome-abgleich.test.mjs tests/maus-absicht.test.mjs > /tmp/maus-schrittgrenze-kaskade.log 2>&1 \
...
node scripts/check-start-lock.mjs --freeze --confirm "Betreiber, 2026-09-07 (Bildschirmfoto): Hinweiszeile Live-Browser verbunden samt dunkler Flaeche entfernen, Seite direkt von ganz oben; nur EIN Mauszeiger, unsere originale Maus in Logo-Tuerkis, alle anderen Zeiger weg. Umgesetzt: bp-hint schwebend und selbst ausblendend, keine Verbunden-Meldung, Zeiger in browser-stage.js tuerkis (#02fdfd, Form icons/maus-zeiger.svg), Maus-Knopf der Tableiste als Geraete-Symbol, #mausButton ausgeblendet. Marken bis zur Wurzel (browser-pane 20260906-11, render/fernwege 20260906-6, stage v7, maus-absicht v35, maus-panel v29, sendepfad-nachladen v19, browser-nachladen v17, app.js b155), Service-Worker smejj-shell-v789. Stempel per Doppelklick im Finder." \
...
if git diff --cached --quiet; then echo "(Manifest unveraendert)"; else
  git commit -q -m "chore(start-lock): Stempel Browser-Oberflaeche 2026-09-07 — app.js b155, index.html, SW smejj-shell-v789 (Betreiber-Doppelklick)
...
git commit -q -m "deploy(browser): Seite ganz oben, schwebender Hinweis, EIN tuerkiser Zeiger, Knoepfe ohne Pfeilform; SW $SW_NEU — Quelle smejj.com-app $QUELLE" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
...
ERW=$(shasum -a 256 "$WT/public/browser-stage.js" | cut -c1-16)
for i in $(seq 1 40); do
  ...
  if [ "$V" = "$SW_NEU" ] && [ "$L" = "$ERW" ]; then
    echo; echo "FERTIG — live: Service-Worker $SW_NEU, browser-stage.js byte-gleich."; exit 0
  fi
  sleep 5
done
