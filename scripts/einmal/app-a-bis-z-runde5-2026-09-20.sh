#!/bin/zsh
# Einmal-Kaskade 18.09.2026, RUNDE 5 — Fenstergroesse ohne neuen Server-Chrome (Worker-Aktion viewport, SW v905):
# BASIS ist der Stand NACH der Zusammenfuehrung mit dem Apple-Login (821f3a17): ausgeliefert wird damit
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock nach dem Browser-A-bis-Z-Livetest gestempelt (SW v905)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
...
ANKER="schutz-100-2026-09-20-app-v905"
