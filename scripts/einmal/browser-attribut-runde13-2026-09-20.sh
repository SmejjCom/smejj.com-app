#!/bin/zsh
# Einmal-Kaskade 20.09.2026, RUNDE 13 — Browser: Adressleiste zeigte t("Suchen (Uebersetzung als nackter Text), iPhone-App: Fenster unter der Statusleiste (SW v924). BASIS = ausgelieferter Stand v923 der i18n-Sitzung (02b46ccb).
...
for d in "$APP" "$BAU"; do
  ...
  if node scripts/check-start-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/frontend/start-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Start-Lock nach dem Browser-A-bis-Z-Livetest gestempelt (SW v924)" || { echo "ABBRUCH: Stempel-Commit $d."; exit 1; }
  fi
  if node scripts/check-security-lock.mjs >/dev/null 2>&1; then
    ...
    git add docs/security/security-lock-manifest.json && git "${autor[@]}" commit -q -m "chore(lock): Sicherheits-Lock nachgestempelt — /api/browser/page als offene Lese-Route, Sprachzeile der Chat-Bruecke (Betreiber 20.09.) (SW v924)" || { echo "ABBRUCH: Sicherheits-Stempel-Commit $d."; exit 1; }
  fi
...
if [ "$LIVE_SW" = "$SW_NEU" ]; then
  ...
  git "${autor[@]}" commit -q -m "deploy(browser): Adressleiste ohne t(-Text, iPhone-App: Fenster unter der Statusleiste behoben; SW $SW_NEU — Quelle smejj.com-app ${APP_NEU:0:8}" || { echo "ABBRUCH: nichts zu committen?"; exit 1; }
...
ANKER="schutz-100-2026-09-20-app-v924"
