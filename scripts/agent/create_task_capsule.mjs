...
const ALLOWED_MODEL_IDS = new Set(["glm-5-2", "kimi-k2-7", "smejj-1-0"]);
...
export function buildTaskCapsule({
  ...
  assertSafeObjectSegment(jobId, "jobId");
  ...
  const prefix = `task-capsules/${year}/${month}/${jobId}`;
  return {
    schemaVersion: 2,
    jobId,
    projectId,
    ...
    provenance: {
      actualProvider: "unresolved",
      actualModel: "unresolved",
      trainingRightsStatus: "not-reviewed",
      sourceRevisions: []
    },
    objects: {
      ...
      trainingEligibility: `${prefix}/training-eligibility.json`,
    ...
    training: {
      ...
      rightsPrivacyQualityGateRequired: true
    }
  };
}

export function writeTaskCapsuleFiles({
  rootDir = process.cwd(),
  outDir,
  jobId,
  projectId,
  ...
  const taskCapsule = buildTaskCapsule({ jobId, projectId, effort, createdAt, modelId });
...
function assertSafeObjectSegment(value, label) {
