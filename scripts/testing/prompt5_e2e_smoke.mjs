...
import { loadSecureLocalEnv, normalizeSecret } from "../../src/shared/env.js";
...
const localSessionSecret = isLocal
  ? normalizeSecret(process.env.SMEJJ_SESSION_SECRET || process.env.GOOGLE_SESSION_SECRET || "")
  : "";
const localAccessToken = localSessionSecret
  ? issueSessionToken({
    secret: localSessionSecret,
    user: { userId: "prompt5-e2e", email: process.env.GOOGLE_ALLOWED_EMAIL || "smejjcom@gmail.com", method: "local-e2e" },
    ttlMs: 10 * 60 * 1000
  })
  : "";
...
await check("brand assets", async () => {
  const assets = new Map([
    ...
    ["/icons/smejj_icon.svg", "image/svg+xml"],
    ["/icons/smejj_favicon.svg", "image/svg+xml"],
    ["/icons/smejj_full_logo.svg", "image/svg+xml"],
    ["/icons/smejj_full_logo_on_dark.svg", "image/svg+xml"],
    ["/icons/pwa-192x192.png", "image/png"],
    ["/icons/pwa-512x512.png", "image/png"],
    ["/icons/maskable-192x192.png", "image/png"],
    ["/icons/maskable-512x512.png", "image/png"]
  ]);
  for (const [path, type] of assets) {
    const { response, text } = await get(path);
    assert(response.ok, `${path} status ${response.status}`);
    assert((response.headers.get("content-type") || "").startsWith(type), `${path} wrong content type`);
    assert(text.length > 0, `${path} empty`);
  }
  return `${assets.size} assets`;
});
