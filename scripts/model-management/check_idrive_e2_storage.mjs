...
function requiredEnv(name) {
  ...
  if (!value) {
    throw new Error(`${name} is required. Set it in the shell or the secure local env file.`);
  }
  return value;
}
...
async function signedS3Get({ endpoint, region, accessKey, secretKey, bucket, prefix }) {
...
const { response, body } = await signedS3Get({
  endpoint,
  region,
  accessKey,
  secretKey,
...
if (!response.ok) {
  console.error(`IDrive e2 list failed: HTTP ${response.status}`);
  console.error(body.slice(0, 500));
  process.exit(1);
}

const keys = parseKeys(body);
console.log(`IDrive e2 list OK: s3://${bucket}/${normalizedPrefix}`);
console.log(`Objects found: ${keys.length}`);
