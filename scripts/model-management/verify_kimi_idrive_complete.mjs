...
const checksumKey = process.env.KIMI_CHECKSUM_KEY || `${prefix}/checksums/streamed-checksums.sha256`;
const config = {
...
if (problems.length > 0) {
  console.error(`Kimi IDrive verification failed with ${problems.length} problem(s):`);
  for (const problem of problems) console.error(`- ${problem}`);
  process.exit(1);
}

console.log("Kimi IDrive verification OK.");
